...
def bivariate_aggregate(
    ...
    work["_x"] = make_x_categories(work[col_x])
...
def main() -> None:
    ...
    with tab_overview:
        ...
        fig_miss.update_xaxes(title_text="שיעור חוסר")
        fig_miss.update_yaxes(title_text="עמודה")
    ...
    with tab_y:
        if COL_Y not in df.columns or COL_PARAM not in df.columns:
            ...
            fig_hist.update_xaxes(title_text="ציון")
            fig_hist.update_yaxes(title_text="כמות")
            ...
            fig_bar.update_xaxes(title_text="משתנה", tickangle=-45)
            fig_bar.update_yaxes(title_text="ממוצע")
    ...
    with tab_context:
        ...
        if not ctx:
            ...
            fig_bar_ctx.update_yaxes(title_text="ממוצע")
            fig_bar_ctx.update_xaxes(title_text=x_choice, tickangle=-25)
    ...
    with tab_pair:
        ...
        if c1 == c2:
            ...
            if g.empty:
                ...
                fig_p.update_xaxes(title_text=kw["x_label"], tickangle=-35)
                fig_p.update_yaxes(title_text=kw["y_label"])
    ...
    with tab_ohe:
        ...
        if pop_cols:
            ...
            fig_p.update_xaxes(title_text="קטגוריה", tickangle=-40)
            fig_p.update_yaxes(title_text="משרדים")
            st.plotly_chart(fig_p, use_container_width=True)
        ...
        if load_cols:
            ...
            fig_l.update_xaxes(title_text="קטגוריה", tickangle=-40)
            fig_l.update_yaxes(title_text="משרדים")
